...
class CoreModule
{
    ...
    public function createInvoice($orderNum, $orderItems, $clientData, $serieInvoices, $clientW, $paymentMethod, $numerocuenta, $usoCFDI)
    {
		...
		$params = array(
			...
			'rfc' => htmlspecialchars_decode($clientData["fiscal-rfc"]),
            'regimen' => $clientData["fiscal-regimen"],
			'calle' => htmlspecialchars_decode($clientData["fiscal-calle"]),
			'numero_exterior' => $clientData["fiscal-exterior"],
			'numero_interior' => $clientData["fiscal-interior"],
			'codpos' => $clientData["fiscal-cp"],
			'colonia' => htmlspecialchars_decode($clientData["fiscal-colonia"]),
			'estado' => htmlspecialchars_decode($clientData["fiscal-estado"]),
			'ciudad' => htmlspecialchars_decode($clientData["fiscal-municipio"]),
			'delegacion' => htmlspecialchars_decode($clientData["fiscal-municipio"]),
			'save' => true,
		...
		//Armamos el objeto completo para facturar.
        $invoiceData = [
